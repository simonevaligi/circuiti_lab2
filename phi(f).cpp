...
void plotDataFromFiles() {
    ...
    TCanvas *c1 = new TCanvas("c1", "Phase vs Frequence", 700, 700);

    // Creazione di un TGraph per ogni set di dati
    TGraph *graph[4];

    // Array per memorizzare i nomi dei file .txt
    std::string fileNames[4] = {"data phi(f)/phi(f)_gen.txt", "data phi(f)/phi(f)_R.txt", "data phi(f)/phi(f)_L.txt", "data phi(f)/phi(f)_C.txt"};
    ...
    leg->AddEntry(graph[0], "#phi_gen", "p");
    leg->AddEntry(graph[1], "#phi_R", "p");
    leg->AddEntry(graph[2], "#phi_L", "p");
    leg->AddEntry(graph[3], "#phi_C", "p");
    leg->Draw(); 
