...
void plotGraphs() {
    ...
    const char* file1 = "VC_VR_Lissajous/VC_VR_3kHz.txt";
    const char* file2 = "VC_VR_Lissajous/VC_VR_resonance.txt";
    const char* file3 = "VC_VR_Lissajous/VC_VR_12kHz.txt";
    ...
    TCanvas* c1 = new TCanvas("c1", "", 400, 650);

    // Impostazione del range degli assi
    graph1->GetXaxis()->SetLimits(xMin - 0.79, xMax + 0.79);
    graph1->SetMinimum(yMin - 0.61);
    graph1->SetMaximum(yMax + 0.61);

    // Disegno del primo grafico
    graph1->SetTitle("VC vs VR;VR (V);VC (V)");
